...
1. `<up><enter>`: enters into ieng6<br />
...
2. `[<cmd> + v] <enter>`: clones the repo<br />
...
3. `cd l<tab>`: enters into the lab7 directory<br />
...
4. (Go to document and highlight commands)`[<cmd> + c]`: (Use mouse and highlight the compile/run commands)<br />
...
5. (Click back to terminal) `[<cmd> + v] <enter>`: runs the JUnit Tests<br />
...
6. `vim L<tab>.<tab><enter>`: opens the ListExamples file in vim<br />
...
7. (click on the '1' on row 43)`a<backspace>2<esc>:wq<enter>`: changes the error, writes to file, and quits<br />
...
8. `[<cmd> + v] <enter>`: runs the JUnit Tests<br />
...
11. `git push`<enter>: pushes edits to your repo\
  Image has add, commit, and push:<br />
